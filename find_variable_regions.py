# First file to run to find variable regions and generate individual dna files.
...
def find_region(sample, input_format, is_pacbio, current_dir, strain_name):
	...
	for line_num, line in enumerate(open(current_dir + "/" + sample, encoding="utf8",errors="ignore")):
		# Finds read name
...
def make_table(strain_name, current_dir):
	...
	table2 = open(current_dir + "/" + strain_name + "_over0count_final_dna_data.csv", "w+")
